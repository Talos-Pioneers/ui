...
const defaultSortOptions = [
	{
		value: 'created_at',
		label: t('components.blueprints.list.sort.createdAt'),
	},
	{
		value: 'updated_at',
		label: t('components.blueprints.list.sort.updatedAt'),
	},
	{ value: 'title', label: t('components.blueprints.list.sort.title') },
	{ value: 'likes_count', label: t('components.blueprints.list.sort.likes') },
	{
		value: 'copies_count',
		label: t('components.blueprints.list.sort.copies'),
	},
]
...
const handleTagFilter = (tagId: string) => {
	emit('update:filter', 'tags.id', [
		...((props.filters['tags.id'] as string[]) || []),
		tagId,
	])
}
...
const unifiedSelectedItems = computed<string[]>(() => {
	...
	if (
		...
		props.filters['tags.id'].length > 0
	) {
	...
	if (
		Array.isArray(props.filters.facility) &&
		props.filters.facility.length > 0
	) {
	...
	if (
		Array.isArray(props.filters.item_input) &&
		props.filters.item_input.length > 0
	) {
	...
	if (
		Array.isArray(props.filters.item_output) &&
		props.filters.item_output.length > 0
	) {
...
const parseUnifiedValue = (
	value: string
): { filterKey: string; originalValue: string } | null => {
	...
	set: (newValues: string[]) => {
		...
		for (const value of added) {
			...
			if (parsed.filterKey === 'region') {
				...
				const currentArray =
					(props.filters[parsed.filterKey] as string[]) || []
				if (!currentArray.includes(parsed.originalValue)) {
					emit('update:filter', parsed.filterKey, [
						...currentArray,
					...
					<SidebarGroupContent>
						<div class="px-2 py-3 space-y-2">
							<label
								...
								>{{
									t(
										'components.blueprints.list.filters.region'
									)
								}}</label
							>
						...
						<div class="px-2 py-3 space-y-2">
							<label
								...
								>{{
									t(
										'components.blueprints.list.filters.facilitiesUsed'
									)
								}}</label
							>
							<SearchableTagsInput
								...
								:placeholder="
									t(
										'components.blueprints.list.filters.facilitiesPlaceholder'
									)
								"
								class="w-full"
							/>
						...
						<div class="px-2 py-3 space-y-2">
							<label
								...
								>{{
									t(
										'components.blueprints.list.filters.inputProducts'
									)
								}}</label
							>
							<SearchableTagsInput
								...
								:placeholder="
									t(
										'components.blueprints.list.filters.inputProductsPlaceholder'
									)
								"
								class="w-full"
							/>
						...
						<div class="px-2 py-3 space-y-2">
							<label
								...
								>{{
									t(
										'components.blueprints.list.filters.outputProducts'
									)
								}}</label
							>
							<SearchableTagsInput
								...
								:placeholder="
									t(
										'components.blueprints.list.filters.outputProductsPlaceholder'
									)
								"
								class="w-full"
							/>
						...
						>
							{{
								sidebarOpen
									...
									: t(
											'components.blueprints.list.filters.show'
										)
							}}
						</Button>
							...
							>
								...
								>
									<SelectValue
										:placeholder="
											t(
												'components.blueprints.list.filters.serverRegion'
											)
										"
									/>
								</SelectTrigger>
						...
						>
							...
							>
								...
								>
									{{
										t(
											'components.blueprints.list.filters.clearAll'
										)
									}}
								</Button>
							</div>
							...
							>
								{{
									t(
										'components.blueprints.list.pagination.found',
						...
						>
							<div class="flex items-center gap-2">
								<label
									...
									>{{
										t(
											'components.blueprints.list.pagination.sortBy'
										)
									}}</label
								>
								<Select
								...
								<Button
									...
									:title="
										isSortDescending
											? t(
													'components.blueprints.list.pagination.sortAscending'
												)
											: t(
													'components.blueprints.list.pagination.sortDescending'
												)
									"
					...
					>
						...
						>
							<p class="text-muted-foreground">
								{{ t('components.blueprints.list.empty') }}
							</p>
						</div>
					</div>
