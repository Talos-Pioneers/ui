...
const submitReport = async () => {
    ...
    try {
        ...
        if (code === 422) {
            const errorObj = error as { response?: { _data?: { message?: string } }; data?: { message?: string } };
            const message = errorObj?.response?._data?.message || errorObj?.data?.message;
    ...
    <Dialog>
        <DialogTrigger as-child>
            <button
                ...
                <span v-if="isReporting">{{ t('reportButton.reporting') }}</span>
        ...
        <DialogContent class="max-w-md">
            ...
            <div class="space-y-4 py-4">
                ...
                <div class="flex items-center justify-end text-xs text-cool-gray-60">
                    <span>{{ remainingCharacters }} {{ t('reportButton.charactersLeft') }}</span>
                </div>
            </div>
            ...
            <DialogFooter>
                ...
                <Button
                    ...
                    @click="submitReport"
                >
                    <span v-if="isReporting">{{ t('reportButton.submitting') }}</span>
