...
const facilities = computed(() => facilitiesData.value?.data.map(f => ({ ...f, icon: `https://static.warfarin.wiki/v1/itemicon/${f.icon}.webp` })) ?? []);
const items = computed(() => itemsData.value?.data.map(i => ({ ...i, icon: `https://static.warfarin.wiki/v1/itemicon/${i.icon}.webp` })) ?? []);
	...
	Object.entries(filters.value).forEach(([key, value]) => {
		...
		if (key === "tags.id" && Array.isArray(value)) {
			for (const item of value) {
				...
				filterTags.push({ filterKey: key, label: tag?.name ?? String(item), value: item });
			}
			return;
		}
		...
		if (key === "facility" && Array.isArray(value)) {
			for (const item of value) {
				...
				filterTags.push({ filterKey: key, label: facility?.name ?? String(item), value: item });
			}
			return;
		}
...
<template>
	<div>
		<BlueprintList :blueprints="blueprints" :pagination="pagination" :facilities="facilities" :items="items"
			...
			@update:filter="setFilter" @update:sort="setSort" @update:current-page="handlePageUpdate"
			@update:per-page="handlePerPageUpdate" @clear-filter="clearFilter" @clear-all-filters="clearAllFilters"
