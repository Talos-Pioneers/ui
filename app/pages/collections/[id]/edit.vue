...
// Form schema matching UpdateBlueprintCollectionRequest
type Schema = {
	title: string
...
// Initialize form with PUT method and existing collection data
const form = usePrecognitionForm<Schema>(
	...
	`/api/v1/collections/${collectionId.value}`,
	{
		...
		status: collection.value?.status || 'draft',
		is_anonymous: collection.value?.creator === null,
	}
)
...
// Initialize form data from collection (only once when collection loads)
const isInitialized = ref(false)
watchEffect(() => {
	if (!collection.value || isInitialized.value) return

	// Update form fields with collection data
	form.fields.title = collection.value.title || ''
	form.fields.description = collection.value.description || null
	form.fields.status = collection.value.status || 'draft'
	form.fields.is_anonymous = collection.value.creator === null
...
const submit = async (status: 'draft' | 'published' = 'draft') => {
	...
	try {
		...
		const message =
			status === 'published'
				? t('pages.collections.edit.successPublic')
				: t('pages.collections.edit.successPrivate')
		...
		if (isValidationError) {
			...
			toast.error(t('pages.collections.edit.errorValidation'))
		} else {
			toast.error(t('pages.collections.edit.errorFailed'))
		}
	} finally {
			...
			<div class="max-w-4xl mx-auto">
				...
				>
					<h1 class="font-bold text-3xl">
						{{ t('pages.collections.edit.title') }}
					</h1>
					...
					>
						<p class="text-sm font-medium text-destructive">
							{{
								t(
									'pages.collections.edit.validationErrorsTitle'
								)
							}}
						</p>
						<ul
					...
					>
						...
						<div class="space-y-2">
							<Label for="title">{{
								t('pages.collections.edit.titleLabel')
							}}</Label>
							<Input
								id="title"
								v-model="form.fields.title"
								:placeholder="
									t('pages.collections.edit.titlePlaceholder')
								"
						...
						<div class="space-y-2">
							<Label for="description">{{
								t('pages.collections.edit.descriptionLabel')
							}}</Label>
							<textarea
								id="description"
								...
								:placeholder="
									t(
										'pages.collections.edit.descriptionPlaceholder'
									)
								"
						...
						<div class="space-y-2">
							<div class="flex items-center space-x-2">
								...
								>
									{{
										t(
											'pages.collections.edit.postAnonymously'
										)
									}}
								</Label>
							</div>
						...
						<div class="flex justify-end gap-3 pt-4">
							...
							>
								<span
									...
									"
									>{{
										t('pages.collections.edit.saving')
									}}</span
								>
								<span v-else>{{
									t('pages.collections.edit.savePrivate')
								}}</span>
							</Button>
							<Button
								type="button"
							...
							>
								<span
									...
									"
									>{{
										t('pages.collections.edit.publishing')
									}}</span
								>
								<span v-else>{{
									t('pages.collections.edit.publishPublic')
								}}</span>
							</Button>
						</div>
					</form>
