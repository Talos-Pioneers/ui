...
const form = usePrecognitionForm<Schema>('put', `/api/v1/blueprints/${blueprintId.value}`, {
...
watchEffect(() => {
  ...
  facilitiesSlugs.value = blueprint.value.facilities?.map(facility => facility.slug) || [];

  // Convert item input IDs to slugs
  itemInputsSlugs.value = blueprint.value.item_inputs?.map(item => item.slug) || [];

  // Convert item output IDs to slugs
  itemOutputsSlugs.value = blueprint.value.item_outputs?.map(item => item.slug) || [];
...
const syncFormFields = () => {
  ...
  form.fields.facilities = facilitiesSlugs.value
      .map(slug => {
        ...
        const existingFacility = blueprint.value?.facilities?.find(f => f.slug === slug);
        return {
  ...
  form.fields.item_inputs = itemInputsSlugs.value
      .map(slug => {
        ...
        // Get quantity from blueprint if it exists
        const existingItem = blueprint.value?.item_inputs?.find(i => i.slug === slug);
        return {
          id: Number(item.id),
  ...
  form.fields.item_outputs = itemOutputsSlugs.value
      .map(slug => {
        ...
        // Get quantity from blueprint if it exists
        const existingItem = blueprint.value?.item_outputs?.find(i => i.slug === slug);
        return {
          id: Number(item.id),
          quantity: existingItem?.quantity || 1,
        };
...
const submit = async (status: 'draft' | 'published' = 'draft') => {
  ...
  try {
    ...
    const response = await sanctumClient<{ data: { id: string } }>(`/api/v1/blueprints/${blueprintId.value}`, {
      method: 'PUT',
      body: formData,
    });
    ...
    <div class="container mx-auto px-4 py-6">
      <div class="max-w-4xl mx-auto">
        <div
          ...
          <h1 class="font-bold text-3xl">{{ t('pages.blueprints.edit.title') }}</h1>
